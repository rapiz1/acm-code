/*
 *     ____              _    
 *    / __ \____ _____  (_)___
 *   / /_/ / __ `/ __ \/ /_  /
 *  / _, _/ /_/ / /_/ / / / /_
 * /_/ |_|\__,_/ .___/_/ /___/
 *            /_/   https://rapiz.me
 *           code@rapiz.me
 *          Fri, 27 Nov 2020 22:38:13 +0800
 */
#include <bits/stdc++.h>
#define xxx(x) cerr<<(#x)<<": "<<x<<endl;
#define fastios ios::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);
typedef long long ll;
using namespace std;
class Solution {
public:
    int pick(int a[], int n, int b[], int m) {
        if (n <= 0) return b[m/2];
        if (m <= 0) return a[n/2];
				if (n + m == 2) return max(a[0], b[0]);
        int am = a[n/2], bm = b[m/2];
				if (am < bm) {
					swap(a, b);
					swap(n, m);
					swap(am, bm);
				}
        if (am > bm) {
					int pa = lower_bound(a, a + n, bm) - a;
					int pb = lower_bound(b, b + m, am) - b - 1;
					return pick(a + pa, n/2 - pa, b + m/2, pb - m/2 + 1);
				}
        else return am;
    }
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
        int *a = new int[nums1.size()];
        copy(nums1.begin(), nums1.end(), a);
        int *b = new int[nums2.size()];
        copy(nums2.begin(), nums2.end(), b);
        double ans = pick(a, nums1.size(), b, nums2.size());
				delete[] a;
				delete[] b;
				return ans;
    }
};
int main() {
	vector<int> a;
	vector<int> b;
	int n, m;
	cin >> n;
	while (n--) {
		int x; cin >> x;
		a.push_back(x);
	}
	cin >> m;
	while (m--) {
		int x; cin >> x;
		b.push_back(x);
	}
	Solution sol;
	cout << sol.findMedianSortedArrays(a, b);
}

